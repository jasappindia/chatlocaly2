...
import com.applozic.mobicomkit.uiwidgets.conversation.chatlocaly.ApiClient;
import com.applozic.mobicomkit.uiwidgets.conversation.chatlocaly.ApiServices;
import com.applozic.mobicomkit.uiwidgets.conversation.chatlocaly.ChatLocallyAppLozicPrefrence;
import com.applozic.mobicomkit.uiwidgets.conversation.chatlocaly.ResponseByUserModel;
import com.applozic.mobicomkit.uiwidgets.conversation.toolbar.Toolbar_ActionMode_Callback;
...
abstract public class MobiComConversationFragment extends Fragment implements View.OnClickListener, GestureDetector.OnGestureListener {
    ...
    public static TextView tv_dateshow;
    ...
    TextView nameTextView, messageTextView, tv_online_offline_textview, tv_message_date;
...
chatLocallyAppLozicPrefrence=new ChatLocallyAppLozicPrefrence(getActivity());
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        tv_dateshow = (TextView) list.findViewById(R.id.tv_message_date);
        tv_dateshow.setVisibility(View.GONE);
        ...
        tv_online_offline_textview = (TextView) list.findViewById(R.id.tv_online_offline_textview);
        tv_message_date = (TextView) list.findViewById(R.id.tv_message_date);
        ...
        listView.setOnScrollListener(new AbsListView.OnScrollListener() {
                                         ...
                                         public void onScroll(AbsListView view, int firstVisibleItem, int amountVisible,
                                             ...
                                             tv_dateshow.setVisibility(VISIBLE);
                                             try {
                                                 if (messageList != null && messageList.size() > 0 && firstVisibleItem + amountVisible > 0) {

                                                     Message message = messageList.get(firstVisibleItem + amountVisible - 2);

                                                     if (DateUtils.isSameDay(message.getCreatedAtTime())) {
                                                         MobiComConversationFragment.tv_dateshow.setText(R.string.today);
                                                     } else {
                                                         MobiComConversationFragment.tv_dateshow.setText(simpleDateFormat.format(new Date(message.getCreatedAtTime())));
...
*/
    ...
    public void loadConversation(final Contact contact, final Channel channel, final Integer conversationId, final String searchString) {
        ...
        if (contact != null && this.channel != null) {
            ...
            tv_online_offline_textview.setText("");
            if (menu != null) {
            ...
            public void run() {
                if (withUserContact != null) {
                    if (withUserContact.isConnected()) {
                        ...
                        tv_online_offline_textview.setText(getActivity().getString(R.string.user_online));

                    } else if (withUserContact.getLastSeenAt() != 0) {

                        tv_online_offline_textview.setText(getActivity().getString(R.string.subtitle_last_seen_at_time) + " " + DateUtils.getDateAndTimeForLastSeen(withUserContact.getLastSeenAt()));
...
//    }
    ...
    public void sendMessage(String message, Map<String, String> messageMetaData, FileMeta fileMetas, String fileMetaKeyStrings, short messageContentType) {
        ...
        new SetResponseByUser(message,messageContentType).execute();
...
// message send
    ...
    public class SetResponseByUser extends AsyncTask<Void, Void, Void> {
        public SetResponseByUser(String message, int messageContentType) {
            ConversationActivity.MESSAGE_SENT = 1;
            ConversationActivity.MESSAGE_SENT_TAG = 1;
            chatLocallyAppLozicPrefrence.setMessageTime(Calendar.getInstance().getTimeInMillis());
            chatLocallyAppLozicPrefrence.setMessageContentType(messageContentType);

            if (messageContentType == 0) {
                chatLocallyAppLozicPrefrence.setMessage(message);
    ...
    public void setResponse() {
        ...
        param.put("encrypt_key", "hbjx");
        param.put("chat_group_id", ConversationActivity.chatGroupId);
        param.put("c_user_id", ConversationActivity.buserId);

       Call<ResponseByUserModel> call = apiServices.addResponse(param);
        call.enqueue(new Callback<ResponseByUserModel>() {
            @Override
            public void onResponse(Call<ResponseByUserModel> call, Response<ResponseByUserModel> response) {
            ...
            @Override
            public void onFailure(Call<ResponseByUserModel> call, Throwable t) {
                ...
                if (isTypingStatus.equals("1")) {
                    if (channel != null) {
                        if (!MobiComUserPreference.getInstance(getActivity()).getUserId().equals(typingUserId)) {
                            ...
                            if (Channel.GroupType.GROUPOFTWO.getValue().equals(channel.getType())) {
                                tv_online_offline_textview.setText(getActivity().getString(R.string.is_typing));

                                ((AppCompatActivity) getActivity()).getSupportActionBar().setSubtitle(getActivity().getString(R.string.is_typing));
                            } else {
                                tv_online_offline_textview.setText(displayNameContact.getDisplayName() + " " + getActivity().getString(R.string.is_typing));
...
//    }
    ...
    private void onListItemSelect(int position) {
        ...
        conversationAdapter.toggleSelection(position);//Toggle the selection
        boolean hasCheckedItems = conversationAdapter.getSelectedCount() > 0;//Check if any items are already selected or not
        //
        if (hasCheckedItems && mActionMode == null)
            // there are some selected items, start the actionMode
            mActionMode = ((AppCompatActivity) getActivity()).startActionMode(new Toolbar_ActionMode_Callback(getActivity(), conversationAdapter, messageList, true));
        ...
        mActionMode.setTitle(String.valueOf(conversationAdapter.getSelectedCount()) + " selected");
