...
public class DetailedConversationAdapter extends ArrayAdapter<Message> {
    ...
    //  meta data  ids for chat information
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //*********************        ashok        *******************/
        // uncomment  code for show  date view in listview
        if (type == 2) {
            ...
            //   MobiComConversationFragment.tv_dateshow.setText(simpleDateFormat.format(date));
...
//applozic code
                        ...
                        // ashok code for open profile ui
                        Intent intent = null;
                        try {
                            intent = new Intent(context, Class.forName("com.chatlocaly.activity.UserProfileActivity"));
                            intent.putExtra(CHAT_USER_ID, message.getContactIds());
                            context.startActivity(intent);
...
//                loadContactImage(receiverContact, contactDisplayName, message, contactImage, alphabeticTextView, onlineTextView);
            ...
            attachmentRetry.setOnClickListener(
...
//                    bgShape.setColor(message.isTypeOutbox() ?
        ...
        View messagesendLayout = customView.findViewById(R.id.messageLayout);

        if (message.getMetadata() != null && message.getMetadata().containsKey(BILL_ORDER_ID)) {


            messagesendLayout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {


                    Log.e("detailed conversation Adapter ", "get message id " + message.getMetadata().get(BILL_ORDER_ID));
                    //    Intent intent=new Intent(context,MainActivity.class);


                    Intent intent;
                    try {
                        intent = new Intent(context, Class.forName("com.chatlocaly.activity.BillShowActivity"));
                        intent.putExtra(BILL_ORDER_ID, message.getMetadata().get(BILL_ORDER_ID));
                        intent.putExtra(BILL_CONFIRMATION_CODE, message.getMetadata().get(BILL_CONFIRMATION_CODE));
                        context.startActivity(intent);
   ...
   // ashok code
                String phonarray[]=data.getTelephoneNumber().split("\n");
                shareContactNo.setText(phonarray[0]);
            } else {
    ...
    public static final String PRODUCT_KEY ="product_name_key" ;
    public static final String SERVICE_KEY ="service_chat_key" ;
        ...
        try {
            ...
            if (mimeType != null) {
                if (mimeType.startsWith("image")) {
                    ...
                    // full view if it is bill type
                    if (smListItem.getMetadata().containsKey(DetailedConversationAdapter.BILL_ORDER_ID)) {

                        Intent intent = null;
                        try {
                            intent = new Intent(context, Class.forName("com.chatlocaly.activity.BillShowActivity"));
                            intent.putExtra(BILL_ORDER_ID, smListItem.getMetadata().get(BILL_ORDER_ID));
                            intent.putExtra(BILL_CONFIRMATION_CODE, smListItem.getMetadata().get(BILL_CONFIRMATION_CODE));
                            context.startActivity(intent);
                        } catch (ClassNotFoundException e) {
                            e.printStackTrace();
                    ...
                    else if(smListItem.getMetadata().containsKey(PRODUCT_KEY))
                    {
                        Intent intent= null;
                        try {
                            intent = new Intent(context,Class.forName("com.chatlocaly.activity.ProductDetailActivity"));
                            intent.putExtra(PRODUCT_KEY,smListItem.getMetadata().get(PRODUCT_KEY));
                            context.startActivity(intent);
                    ...
                    else if(smListItem.getMetadata().containsKey(SERVICE_KEY)){

                        Intent intent= null;
                        try {
                            intent = new Intent(context,Class.forName("com.chatlocaly.activity.ServiceDetailActivity"));
                            intent.putExtra(SERVICE_KEY,smListItem.getMetadata().get(SERVICE_KEY));
                            context.startActivity(intent);
    ...
    public boolean isSameSelect(int position) {
